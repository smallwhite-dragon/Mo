    ...
    def predict(self,
        ...
        if self.config.use_python_inference:
            ...
            for i in range(warmup):
                ...
                boxes_tensor = self.predictor.get_input_handle(output_names[0])
                np_boxes = boxes_tensor.copy_to_cpu()
                if self.config.mask_resolution is not None:
                    masks_tensor = self.predictor.get_output_handle(
                        output_names[1])
                    np_masks = masks_tensor.copy_to_cpu()

                if self.config.with_lmk is not None and self.config.with_lmk == True:
                    face_index = self.predictor.get_output_handle(output_names[
                        1])
                    landmark = self.predictor.get_output_handle(output_names[2])
                    prior_boxes = self.predictor.get_output_handle(output_names[
                        3])
...
# 结果根据外壳与呼吸膜阈值不同进行过滤
def filter_threshold(results):
    # print('----aaaa', results)
    nboxs = results['boxes']
    nboxs_new = []
    # print(nboxs.shape)
    # -----------------------
    thresh_cov = FLAGS.threshold
    thresh_hxm = FLAGS.threshold_max
    # -----------------------
    for box in nboxs:
        classid = int(box[0])
        confd = box[1]
        if classid == 0 and confd > thresh_cov:
            # print('thresh_cov =', confd)
            nboxs_new.append(box)
        elif classid == 1 and confd > thresh_hxm:
            # print('thresh_hxm =', confd)
            nboxs_new.append(box)
    nboxs_new = np.asarray(nboxs_new)
    # print(nboxs_new.shape)
    nboxs_new.reshape(-1, 6)
    results['boxes'] = nboxs_new
    # print('+++++aaaa', results)
    return results
...
def predict_image(detector):
    ...
    if FLAGS.run_benchmark:
        ...
        results = filter_threshold(results)
        if FLAGS.visual:
            img = visualize(
                FLAGS.image_file,
                results,
...
def predict_video(detector, camera_id, video_file):
    if camera_id != -1:
...
# 2021-8-7 add threshold_max
class Flag:
    def __init__(self, model_dir, image_file, video_file, camera_id,
                 run_mode, use_gpu, run_benchmark, threshold, threshold_max, output_dir, visual):
...
def buildDetector(model_dir=None, image_file=None, video_file=''
         , camera_id=-1, run_mode='fluid'
         , use_gpu=False, run_benchmark=False
         , threshold=0.5, threshold_max=0.9, output_dir='output', visual=True):
    global FLAGS
    res = None
    FLAGS = Flag(model_dir, image_file, video_file, camera_id,
                run_mode, use_gpu, run_benchmark,
                threshold=threshold,threshold_max=threshold_max, output_dir=output_dir, visual=visual)

    if FLAGS.image_file != '' and FLAGS.video_file != '':
        assert "Cannot predict image and video at the same time"
    config = PredictConfig(FLAGS.model_dir)
    detector = Detector(
    ...
    # predict from image
    #if FLAGS.image_file is not None and len(FLAGS.image_file)>0:
    #    return predict_image(detector)
...
def getRes(image_file=None,detector=None):
    global FLAGS
    if detector is None:
        assert "detector is null"
    else:
        if image_file is not None and len(image_file) > 0:
            FLAGS.image_file = image_file
            res, image = predict_image(detector)
            return res, image
    return None, None


if __name__ == '__main__':
    detector = buildDetector(model_dir='D:/36ke/model/0818')
    rres, iimg = getRes(image_file='D:/36ke/image/cp1628219260NG.jpg', detector=detector)


    # cv2.imwrite("D:/36ke/model" + str(time.ctime()) + str(rres) + ".jpg", iimg)
    # print("D:/36ke/model" + str(time.ctime())  + ".jpg")
    # nboxs = rres['boxes']
    # for box in nboxs:
    #     classid = int(box[0])
    #     confd = box[1]
