...
class MainWindow(QWidget):
    ...
    def __init__(self):
        ...
        t1 = time.time()
        self.detector = buildDetector(model_dir='model/hxm-4820', threshold=0.5, use_gpu=True, run_mode='fluid')
    ...
    def add_model(self, trigger_text):
        ...
        if trigger_text == 'Please select a model':
            ...
            if os.path.exists(model_path_model):
                self.cam_flag = False
                self.detector = buildDetector(model_dir=model_path, threshold=0.9, use_gpu=True, run_mode='fluid')
                self.cam_flag = True
