...
class Flag:
    def __init__(self, model_dir, image_file, video_file, camera_id,
                 run_mode, use_gpu, run_benchmark, threshold, output_dir, visual):
...
def buildDetector(model_dir=None, image_file=None, video_file=''
         , camera_id=-1, run_mode='trt_fp16'
         , use_gpu= True , run_benchmark=False
         , threshold=0.5, output_dir='output', visual=True):
    global FLAGS
    res = None
    FLAGS = Flag(model_dir, image_file, video_file, camera_id,
                run_mode, use_gpu, run_benchmark,
                threshold, output_dir, visual)
...
if __name__ == '__main__':
    detector = buildDetector(model_dir='model')
    rres, iimg = getRes(image_file='cp1621988395.jpg', detector=detector)
    print(rres)
    #covers = []
    #huximos = []
    #score = 0.0
    #for re in rres['boxes']:
    #    print(re)
	#            if re['score'] < thresh:
	#               continue
    #    score += re[1]                                                              #score
